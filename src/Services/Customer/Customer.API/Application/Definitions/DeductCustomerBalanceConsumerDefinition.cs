...
{
    public class DeductCustomerBalanceConsumerDefinition : ConsumerDefinition<DeductCustomerBalanceConsumer>
    {
        public DeductCustomerBalanceConsumerDefinition()
        {
            // Map to the service-level queue to match Orchestration mapping
            EndpointName = QueueNames.CustomerService;
        }

        protected override void ConfigureConsumer(IReceiveEndpointConfigurator endpointConfigurator, IConsumerConfigurator<DeductCustomerBalanceConsumer> consumerConfigurator, IRegistrationContext context)
        {
