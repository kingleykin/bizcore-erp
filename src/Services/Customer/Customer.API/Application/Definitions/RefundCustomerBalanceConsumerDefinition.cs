...
{
    public class RefundCustomerBalanceConsumerDefinition : ConsumerDefinition<RefundCustomerBalanceConsumer>
    {
        public RefundCustomerBalanceConsumerDefinition()
        {
            // Map to the service-level queue to match Orchestration mapping
            EndpointName = QueueNames.CustomerService;
        }

        protected override void ConfigureConsumer(IReceiveEndpointConfigurator endpointConfigurator, IConsumerConfigurator<RefundCustomerBalanceConsumer> consumerConfigurator, IRegistrationContext context)
        {
